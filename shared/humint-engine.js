...
(function () {
  ...
  function initMap() {
    ...
    whenStyleLoaded(function () {
      map.resize();
      // Frontières : on masque la fine native (non épaississable) et on trace les
      // nôtres en noir depuis un geojson aligné (lignes Natural Earth 50m, sans côtes).
      try { map.setConfigProperty('basemap', 'colorAdminBoundaries', 'hsla(0,0%,0%,0)'); } catch (e) { /* config indispo */ }
      try {
        map.addSource('borders-geo', { type: 'geojson', data: 'africa-bounds.geojson' });
        map.addLayer({
          id: 'borders-thick', type: 'line', slot: 'middle', source: 'borders-geo',
          layout: { 'line-join': 'round', 'line-cap': 'round' },
          paint: {
            'line-color': '#000000',
            'line-width': ['interpolate', ['linear'], ['zoom'], 2, 1.1, 6, 1.9, 9, 2.7, 12, 3.4],
            'line-opacity': 0.85,
          },
        });
