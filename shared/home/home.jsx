...
function useSubscriber(){
  ...
  useEffect(()=>{ let on=true;
    const check=()=>{ try{ const a=window.algorAuth; if(a&&a.supabase&&a.supabase.auth&&a.supabase.auth.getSession){ a.supabase.auth.getSession().then(({data})=>{ if(on) setSub(!!(data&&data.session)); }); } }catch(e){} };
    check(); window.addEventListener('algorAuthReady',check);
    return ()=>{on=false; window.removeEventListener('algorAuthReady',check);};
  },[]);
  return sub;
}
