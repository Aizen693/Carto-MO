...
const { useEffect: useEffectGlobe, useRef: useRefGlobe } = React;
...
function Globe() {
  ...
  useEffectGlobe(() => {
    ...
    // — anchors (theaters)
    const anchors = [
      { id: 'MO-01',    lon:  44,   lat:  33,   label: 'MO-01 · Moyen-Orient' },
      { id: 'SAHEL-02', lon:   0,   lat:  16,   label: 'SAHEL-02 · Sahel' },
      { id: 'LACS-03',  lon:  29,   lat:  -2,   label: 'LACS-03 · Grands Lacs' },
      { id: 'MDG-04',   lon:  47,   lat: -19,   label: 'MDG-04 · Madagascar' },
      { id: 'AFR-05',   lon:  43,   lat:  12,   label: 'AFR-05 · Afrique Maritime' },
      { id: 'ASIE-06',  lon:  74,   lat:  30,   label: 'ASIE-06 · Asie du Sud' }
    ];
    ...
    function draw(t) {
      ...
      anchors.forEach((a, i) => {
        ...
        ctx.beginPath(); ctx.arc(xy[0], xy[1], 8.5, 0, Math.PI * 2); ctx.stroke();
        ctx.strokeStyle = `rgba(${C.gold},0.30)`;
        ctx.beginPath(); ctx.arc(xy[0], xy[1], 12 + pulse * 3, 0, Math.PI * 2); ctx.stroke();

        // halo dore serre
        ctx.beginPath(); ctx.arc(xy[0], xy[1], 5.4, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${C.gold},0.92)`;
        ctx.fill();

        // coeur blanc lumineux — plus gros + plus brillant que les flux
        ctx.beginPath(); ctx.arc(xy[0], xy[1], 3.4, 0, Math.PI * 2);
    ...
    let rafId = 0;
    function frame(now) { step(now); rafId = requestAnimationFrame(frame); }
    ...
    function onLeave() { canvas.style.cursor = 'default'; zoomTarget = null; }
    function onClick(e) {
      const rect = canvas.getBoundingClientRect();
      const hit = findAnchorAt(e.clientX - rect.left, e.clientY - rect.top);
